...
const NativeView = requireNativeViewManager<Props>("ExpoApplePay");

export type ApplePayButtonRef = {
  onTokenSuccess: () => Promise<void>;
  onTokenFailed: () => Promise<void>;
};

const ExpoApplePay = React.forwardRef<ApplePayButtonRef, Props>((props, ref) => {
  const {
    type = "plain",
    buttonStyle = "black",
    cornerRadius = 4,
